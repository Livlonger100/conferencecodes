...
export const metadata: Metadata = {
  title: "Longevity Conference Discount Codes 2026 — Health & Biotech Events | ConferenceCodes",
  description:
    "Exclusive discount codes for longevity, health, and biotech conferences in 2026. Verified pricing and promo codes for Longevity Summit Dublin, Singularity Summit South Africa, and more.",
};

const NAV_LINKS = [
  { href: "/ai-conferences", label: "AI Conferences", active: false },
  { href: "/longevity-conferences", label: "Longevity", active: true },
  { href: "/how-it-works", label: "How It Works", active: false },
  { href: "/for-organizers", label: "For Organizers", active: false },
];

export default async function LongevityConferencesPage() {
  const { data } = await supabaseAdmin
    .from("conferences")
    .select("*, pricing_tiers(*), hotels(*)")
    .eq("category", "Longevity / Health")
    ...
    <div style={{ minHeight: "100vh", background: "#f8f9fa", fontFamily: "'DM Sans', -apple-system, sans-serif", color: "#111827" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .conf-card:hover { border-color: rgba(52,211,153,0.4) !important; transform: translateY(-2px); box-shadow: 0 8px 30px rgba(0,0,0,0.08) !important; }
      ...
      <div style={{ background: "#f8f9fa", borderBottom: "1px solid #e5e7eb", padding: "40px 32px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <span style={{ display: "inline-block", fontSize: 11, fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase", color: "#34d399", background: "rgba(52,211,153,0.1)", padding: "4px 10px", borderRadius: 5, marginBottom: 14 }}>Longevity / Health</span>
          <h1 style={{ fontSize: 36, fontWeight: 800, color: "#111827", margin: "0 0 12px 0", lineHeight: 1.2 }}>
            Longevity Conference Discount Codes 2026
          </h1>
          <p style={{ fontSize: 16, color: "#6b7280", margin: "0 0 20px 0", maxWidth: 600, lineHeight: 1.6 }}>
            {conferences.length} verified longevity, health, and biotech conferences with exclusive promo codes, early bird pricing, and negotiated hotel rates.
          </p>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 32px 80px" }}>
              ...
              <a key={conf.id} href={`/conference/${conf.slug}`} className="conf-card" style={{ display: "block", background: "#fff", border: "1px solid #e5e7eb", borderRadius: 16, padding: 24, boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 24 }}>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    {price > 0 && (
                      <div style={{ fontSize: 24, fontWeight: 800, color: "#34d399", marginBottom: 4 }}>{formatPrice(price)}</div>
                    )}
                    {conf.earlyBird && (
                      <div style={{ fontSize: 11, color: "#34d399", marginBottom: 8 }}>Early bird</div>
                    )}
                    <span style={{ fontSize: 13, fontWeight: 600, color: "#34d399" }}>View details →</span>
                  </div>
                </div>
              </a>
            );
        ...
        <div style={{ textAlign: "center", marginTop: 48 }}>
          <a href="/" style={{ display: "inline-flex", alignItems: "center", gap: 8, fontSize: 15, fontWeight: 600, color: "#f97316", padding: "12px 28px", border: "1px solid rgba(249,115,22,0.3)", borderRadius: 10, background: "rgba(249,115,22,0.05)" }}>
            View all conferences including AI / Tech →
          </a>
        </div>
      </div>
